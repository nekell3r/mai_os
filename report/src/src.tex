...
\textbf{Параметры:}
\begin{itemize}
    \item \texttt{lpLibFileName} — путь к DLL файлу (например, "bubble\_sort.dll")
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{lpProcName} — имя функции (например, "Sort")
\end{itemize}
...
\subsection{Контракт функции сортировки}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{bubble\_sort.dll} — реализация пузырьковой сортировки
        \item \texttt{quicksort.dll} — реализация сортировки Хоара
    \end{itemize}
    ...
    \begin{itemize}
        \item \texttt{program1.exe} — статическая линковка (использует bubble\_sort)
        \item \texttt{program2.exe} — динамическая загрузка (переключение между DLL)
    \end{itemize}
...
Программа использует пузырьковую сортировку, которая статически слинкована на этапе компиляции.

\textbf{Особенности:}
\begin{itemize}
    \item Код сортировки включен в .exe файл
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Если команда "1" — парсинг чисел и вызов \texttt{Sort()}
        \item Вывод отсортированного массива
...
Программа загружает DLL во время выполнения и может переключаться между разными реализациями.

\textbf{Особенности:}
\begin{itemize}
    \item Класс \texttt{DynamicLibrary} для управления загрузкой DLL
    \item Переключение между \texttt{bubble\_sort.dll} и \texttt{quicksort.dll} командой "0"
...
\begin{enumerate}
    \item Вывод приветственного сообщения
    \item Загрузка начальной библиотеки (\texttt{bubble\_sort.dll})
    \item Цикл обработки команд:
    \begin{enumerate}
        \item Чтение команды из консоли
        ...
        \begin{itemize}
            ...
            \item Получение адреса функции (\texttt{GetProcAddress()})
        \end{itemize}
        \item Если команда "1" — парсинг чисел и вызов функции через указатель
        \item Вывод отсортированного массива
...
public:
    ...
    SortFunc getSortFunc() const;           // Получение указателя
};
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item Сохранение handle и указателя на функцию
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Сброс handle и указателя на функцию
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{GetProcAddress()} — проверка указателя на функцию
    \item \textbf{Валидация входных данных} — проверка размера массива (максимум 1000 элементов)
...
\begin{itemize}
    \item \texttt{bubble\_sort.dll} и \texttt{quicksort.dll} в \texttt{build/}
